...
legacy_demo_group_names = [ "Demo Feed Owners", "Demo Screen Owners" ]
legacy_demo_content_names = [
  "Welcome Ticker",
  "HTML Ticker",
  "GenAI Poster",
  "Welcome Robot",
  *(1..6).map { |index| "Landscape #{index}" }
]

Group.where(name: legacy_demo_group_names).find_each(&:destroy!)
Content.where(user: system_user, name: legacy_demo_content_names).find_each(&:destroy!)
...
if template.persisted? && Setting[:default_template_id].blank?
  Setting[:default_template_id] = template.id
end
