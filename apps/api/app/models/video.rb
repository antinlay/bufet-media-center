...
class Video < Content
  ...
  after_commit :enqueue_transcode, on: [ :create, :update ]
  ...
  def playback_url
    if mp4.attached?
      rails_blob_path(mp4, only_path: true)
    elsif file.attached?
  ...
  def thumbnail_url
    if poster.attached?
      rails_blob_path(poster, only_path: true)
    elsif video_source == "youtube"
      "https://img.youtube.com/vi/#{video_id}/mqdefault.jpg"
    elsif video_source == "vimeo"
      data = vimeo_oembed_data
      data&.dig("thumbnail_url").presence
    elsif video_source == "tiktok"
      data = tiktok_oembed_data
      data&.dig("thumbnail_url").presence
    else
  ...
  def file_content_type
    ...
    return if content_type&.start_with?("video/")
  ...
  def file_size_within_limit
    ...
    if file.blob.byte_size > MAX_FILE_SIZE
      errors.add(:file, "is too large (max 100MB)")
    end
  end

  def enqueue_transcode
    return unless file.attached?
    return if mp4.attached? && poster.attached?

    VideoTranscodeJob.perform_later(id)
  rescue StandardError => e
    Rails.logger.error("Video transcode enqueue failed for Video #{id}: #{e.message}")
  end
